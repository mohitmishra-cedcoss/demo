...
if( !class_exists( 'MWB_WPR_Front_End' ) )
{
	...
	class MWB_WPR_Front_End{
		...
		public function add_hooks_and_filters()
		{
			...
			add_action( 'user_register' , array( $this, 'update_welcome_membership' ), 10, 1);
			add_action('woocommerce_before_add_to_cart_form',array($this,'mwb_display_product_points'),7);
			...
			// add_action('woocommerce_download_product', array($this,'mwb_get_coins_on_download'),10, 6);
			

		}
		public function mwb_get_coins_on_download($user_email, $order_key, $product_id, $user_id, $download_id, $order_id){
		...
		*/
		public function mwb_display_product_points(){
			...
			if(is_user_logged_in() && $this->has_membership()) {
					$user_id = get_current_user_ID();
					$price_product = $product->get_price();
					$points = $this->get_points_calculation($user_id,$price_product);
					echo "<span style=background-color:".$mwb_wpr_notification_color.">".$mwb_wpr_assign_pro_text.": ".$points;_e(' Points',MWB_WPR_Domain);
						//echo $img;
						echo "</span>";
		...
		/**
		 * This function used to get check the membership of the user.
		 * 
		 * @name has_membership
		 * @author makewebbetter<webmaster@makewebbetter.com>
		 * @link http://www.makewebbetter.com/
		*/
		...
		/**
		 * This function used to update the welcome membership of the registred  user.
		 * 
		 * @name has_membership
		 * @author makewebbetter<webmaster@makewebbetter.com>
		 * @link http://www.makewebbetter.com/
		*/
		public function update_welcome_membership($user_id) {
			$membership_settings_array = get_option('mwb_wpr_membership_settings',true);
			...
			$welcome_membership = isset($membership_settings_array['welcome_membership']) ? $membership_settings_array['welcome_membership'] :'';
			$mwb_wpr_membership_roles = isset($membership_settings_array['membership_roles']) && !empty($membership_settings_array['membership_roles']) ? $membership_settings_array['membership_roles'] : array();
			$today_date = date_i18n("Y-m-d h:i:sa");
			$mwb_wpr_notificatin_array=get_option('mwb_wpr_notificatin_array',true);
			$user=get_user_by('ID',$user_id);
			$user_email=$user->user_email;
			$membership_detail = get_user_meta($user_id, 'points_details',true);
			if(!empty($mwb_wpr_mem_enable) && $mwb_wpr_mem_enable == 1) {
				if (!empty($welcome_membership) && !empty($mwb_wpr_membership_roles) && is_array($mwb_wpr_membership_roles)) {
					
					foreach ($mwb_wpr_membership_roles as $roles => $values) {
						if( !empty($welcome_membership) && $welcome_membership == $roles ) {
							...
							update_user_meta($user_id,'membership_level',$welcome_membership);
							...
							if(is_array($mwb_wpr_notificatin_array) && !empty($mwb_wpr_notificatin_array))
							{
								...
								$mwb_wpr_email_discription=str_replace("[USERLEVEL]",$welcome_membership,$mwb_wpr_email_discription);
								if($mwb_wpr_notificatin_enable)
								{	
									$headers = array('Content-Type: text/html; charset=UTF-8');
									wc_mail($user_email,$mwb_wpr_email_subject,$mwb_wpr_email_discription,$headers);
		...
		*/
		public function mwb_wpr_new_customer_registerd( $customer_id, $new_customer_data, $password_generated ){	
			...
			//==================update_welcome_membership===========
				$this->update_welcome_membership($customer_id);
			//==================update_welcome_membership===========

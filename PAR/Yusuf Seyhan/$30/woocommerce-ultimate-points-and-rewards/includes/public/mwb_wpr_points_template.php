...
if(isset($_POST['mwb_wpr_save_level']))
{
	...
	// print_r($mwb_wpr_membership_roles);die;
...
function get_select_levels_array() {

	$user_id = get_current_user_id();
	$mwb_wpr_membership_array = array();
		...
		if(!empty($mwb_wpr_membership_roles) && is_array($mwb_wpr_membership_roles)) {
			...
			foreach( $mwb_wpr_membership_roles as $roles => $values) {
				if($roles == $selected_role) {
					...
					$mwb_wpr_membership_array[$selected_role] = $values['Points'];
				}
			}
		}

	}
	return $mwb_wpr_membership_array;
}
$user_id = get_current_user_id();
$get_points = (int)get_user_meta($user_id, 'mwb_wpr_points', true);
...
$membership_settings_array = get_option('mwb_wpr_membership_settings',true);
$mwb_wpr_membership = isset($membership_settings_array['membership_roles']) && !empty($membership_settings_array['membership_roles']) ? $membership_settings_array['membership_roles'] : array();
if(!empty($mwb_wpr_membership) && is_array($mwb_wpr_membership) && !empty($user_level)) {
	
	foreach ($mwb_wpr_membership as $roles => $values) {
		if($user_level == $roles) {
			$mwb_wpr_badges_url = $values['badges'];
		}
...
?>	
	<div class="mwb_wpr_heading"><?php _e('Membership List', MWB_WPR_Domain); ?>
		<?php if(!empty($mwb_wpr_badges_url)):?>
	   <div class="mwb_wpr_myaccount_badge">
		 <img  width="100" height="100" src="<?php echo $mwb_wpr_badges_url; ?>">
	   </div>
       <?php endif; ?>
	</div>
...
<?php
	...
	if(isset($enable_drop) && $enable_drop)
	{
		...
		$mwb_wpr_membership_array = get_select_levels_array();

		if(!empty($mwb_wpr_membership_array) && is_array($mwb_wpr_membership_array) 
			&& !empty($user_level)) {


			foreach ($mwb_wpr_membership_roles as $roles => $values) {
				
				if( $mwb_wpr_membership_array[$user_level] > $values['Points'] )
				{
					unset($mwb_wpr_membership_roles[$roles]);
				}
			}
		}

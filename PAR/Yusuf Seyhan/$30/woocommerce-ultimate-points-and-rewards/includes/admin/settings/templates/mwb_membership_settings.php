...
if(isset($_POST['mwb_wpr_save_membership']))
{
	...
	if($current_tab=="mwb_wpr_membership_tab")
	{
		...
		//=======custmization===================================
		$welcome_membership = isset($_POST['mwb_wpr_membership_list']) ? $_POST['mwb_wpr_membership_list'] :'';
		//=======custmization===================================
		...
		$membership_settings_array['welcome_membership'] = $welcome_membership;
...
?>
	...
	<table class="form-table mwb_wpr_membership_setting mwp_wpr_settings">
		...
		<tbody>
			...
			<!-- Start Custom Code For the Welocme Membership -->
			<?php
			if(!empty(is_array($mwb_wpr_membership_roles) && !empty($mwb_wpr_membership_roles))) :
				?>
			<tr>
				<th scope="row" class="titledesc">
					<label for="mwb_wpr_membership_product_list"><?php _e('Select Memebrship For Welocme Memebrship', MWB_WPR_Domain)?></label>
				</th>
				<td class="forminp forminp-text">
					<?php 
					$attribute_description = __('Selected membership will get assigned to the registed user.', MWB_WPR_Domain);
					echo wc_help_tip( $attribute_description );
					?>
					<select id="mwb_wpr_membership_list" class="wc-enhanced-select " name="mwb_wpr_membership_list">
						<?php
						foreach ($mwb_wpr_membership_roles as $role => $value) {
							?>
							<option value="<?php echo $role; ?>" <?php selected( $welcome_membership ,$role)?>><?php echo $role; ?></option>";
							<?php 
						} 
						?>
					</select>
			...
			<?php endif;?>
			<!-- End Custom Code For the Welocme Membership -->
			<tr valign="top">
				<th scope="row" class="titledesc">

...
if( !class_exists( 'MWB_WPR_Front_End' ) )
{
	...
	class MWB_WPR_Front_End{
		...
		public function add_hooks_and_filters()
		{
			...
			add_filter('woocommerce_registration_errors',array($this,'check_usrname_is_number'),10,3);
			...
			add_filter( 'woocommerce_add_cart_item_data', array($this, 'mwb_wpr_woocommerce_add_cart_item_data'), 10, 4);
		...
		public function check_usrname_is_number($errors, $username, $email) {
			if(!empty($username) && is_numeric($username)) {
				return $errors;

			}
			elseif (!is_numeric($username)) {
				return new WP_Error( 'registration-error', __( 'Please enter username in the numbers.',MWB_WPR_Domain) );
			}
		}
		/**
		...
		*/
		...
		/**
		 * This function is used to check is order total is enable or not
		 * 
		...
		*/
		public function check_enable_offer(){
			...
			if(!empty($thankyouorder_enable) && $thankyouorder_enable == "on") {
				$is_enable = true;
			}
			return $is_enable;
		}
		...
		/**
		 * This function is used to calculate the order total points from the order
		...
		*/
		...
		if(!empty($mwb_wpr_point)) {
			$this->generate_points_log($user_id,$mwb_wpr_point);
			$this->send_mail_order_total($user_id,$mwb_wpr_point);
		}
	}

	/**
	* This function is used to generate points log of the order total
	* @name generate_points_log
	* @author makewebbetter<webmaster@makewebbetter.com>
	* @link http://www.makewebbetter.com/
	* @param $purchase_value,$user_id
	*/
	public function generate_points_log($user_id,$purchase_value) {
		$mwb_referral_purchase_value = $purchase_value;
		$ref_product_detail_points = get_user_meta($user_id, 'points_details',true);
		$today_date = date_i18n("Y-m-d h:i:sa");
		if(isset($ref_product_detail_points['points_on_order']) && !empty($ref_product_detail_points['points_on_order']))
		{		
			$ref_pro_array = array(
				'points_on_order'=>$mwb_referral_purchase_value,
				'date'=>$today_date,
				);
			$ref_product_detail_points['points_on_order'][] = $ref_pro_array;
		}			
		else
		{
		...
		update_user_meta( $user_id , 'points_details' , $ref_product_detail_points );
	}

	/**
	* This function is used to send mail for the order total point.
	* @name send_mail_order_total
	* @author makewebbetter<webmaster@makewebbetter.com>
	* @link http://www.makewebbetter.com/
	* @param $purchase_value,$user_id
	*/
	public function send_mail_order_total($user_id,$points) {
		$mwb_wpr_notificatin_array=get_option('mwb_wpr_notificatin_array',true);
		$total_points = (int)get_user_meta($user_id,'mwb_wpr_points',true);
		$user = get_user_by('ID',$user_id);
		$user_email=$user->user_email; 
		...
		{
			$mwb_wpr_notificatin_enable=isset($mwb_wpr_notificatin_array['mwb_wpr_notificatin_enable']) ? intval($mwb_wpr_notificatin_array['mwb_wpr_notificatin_enable']) : 0;
			$mwb_wpr_email_subject=isset($mwb_wpr_notificatin_array['mwb_wpr_point_on_order_total_range_subject'])? $mwb_wpr_notificatin_array['mwb_wpr_point_on_order_total_range_subject'] :'';
			$mwb_wpr_email_discription=isset($mwb_wpr_notificatin_array['mwb_wpr_point_on_order_total_range_desc']) ? $mwb_wpr_notificatin_array['mwb_wpr_point_on_order_total_range_desc'] :'';
				// $mwb_wpr_email_discription=str_replace("[Points]",$mwb_referral_purchase_value,$mwb_wpr_email_discription);

				// print_r($total_points);die("fkj");
			...
			$mwb_wpr_email_discription=str_replace("[ORDERTOTALPOINT]",$points,$mwb_wpr_email_discription);
				// $mwb_wpr_email_discription=str_replace("[Per Currency Spent Points]",$mwb_per_currency_spent_value,$mwb_wpr_email_discription);
			$user = get_user_by('email',$user_email);
			$user_name = $user->user_firstname;
			$mwb_wpr_email_discription = str_replace("[USERNAME]",$user_name,$mwb_wpr_email_discription);
						...
						foreach ( $coupons as $item_id => $item ) :
							foreach ($user_log as $key => $value) {
								if(in_array(strtoupper($item->get_code()), $value)) {
									...
									$couponamont = $couponamont+$item->get_discount();
        ...
        public function mwb_overwrite_form_temp($path, $template_name) {
        	...
        	if($mwb_wpr_apply_points_checkout == 1){
        		if( $template_name == 'myaccount/form-login.php') {
        			return MWB_WPR_DIRPATH.'woocommerce/myaccount/form-login.php';
        		}
        	}
        	return $path;
        }

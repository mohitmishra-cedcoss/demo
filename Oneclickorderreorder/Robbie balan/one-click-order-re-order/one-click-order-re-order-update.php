...
if( ! class_exists( 'One_Click_Order_Re_order_Update' ) ) 
{
	class One_Click_Order_Re_order_Update 
	{
		public function __construct() {
			register_activation_hook(ONE_CLICK_ORDER_RE_ORDER_BASE_FILE, array($this, 'mwb_check_activation'));
			add_action('mwb_check_event', array($this, 'mwb_check_update'));
			add_filter( 'http_request_args', array($this, 'mwb_updates_exclude'), 5, 2 );
			register_deactivation_hook(ONE_CLICK_ORDER_RE_ORDER_BASE_FILE, array($this, 'mwb_check_deactivation'));
		}	
		...
		public function mwb_check_update() 
		{
			global $wp_version;
			global $update_check;
			$plugin_folder = plugin_basename( dirname( ONE_CLICK_ORDER_RE_ORDER_BASE_FILE ) );
			$plugin_file = basename( ( ONE_CLICK_ORDER_RE_ORDER_BASE_FILE ) );
			...
			$postdata = array(
				'action' => 'check_update',
				'license_key' => ONE_CLICK_ORDER_RE_ORDER_LICENSE_KEY
			);
		...
		public function mwb_plugin_get($i) 
		{
			...
			$plugin_folder = get_plugins( '/' . plugin_basename( dirname( ONE_CLICK_ORDER_RE_ORDER_BASE_FILE ) ) );
			$plugin_file = basename( ( ONE_CLICK_ORDER_RE_ORDER_BASE_FILE ) );
			return $plugin_folder[$plugin_file][$i];
		}
	}
	new One_Click_Order_Re_order_Update();
}		
?>
 ...
 * @version     2.1.0
 */
class WC_Report_Sales_By_Product_testing extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s sales for the selected items', 'woocommerce' ), '<strong>' . wc_price(10 ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s purchases for the selected items', 'woocommerce' ), '<strong>' . (200) . '</strong>' ),
	...
	public function products_widget() {
		...
		<div class="section">
			<table cellspacing="0">
				...
				$mwb_cpr_user_referred_by = array();
				$mwb_crp_user_array = array();
				foreach ($users as $key => $user) {	
				  ...
				  if(!empty($referral_user_data)) {
					  	...
					  	$mwb_cpr_user_referred_by[] = get_user_meta($refree_id,'mwb_cpr_user_referred_by',true);
					  	$mwb_crp_user_array[]= array($user->ID=>get_user_meta($refree_id,'mwb_cpr_user_referred_by',true)); 
				  	}
				}
				// print_r($mwb_cpr_user_referred_by);
				// print_r($mwb_crp_user_array);die();
				// print_r($top_sellers);
				if ( $top_sellers ) {
					// @codingStandardsIgnoreStart

...
class Coupon_Referral_Program_Public {
	...
	public function woocommerce_created_customer_discount($user_id){
		if($this->check_signup_is_enable()){
		...
		if($enable_plugin){
			$cookie_val = isset($_COOKIE['mwb_cpr_cookie_set']) ? $_COOKIE['mwb_cpr_cookie_set'] : '';
			...
			if(!empty($retrive_data)){				
				...
				if(isset($refree_id) && !empty($refree_id) && !empty($user_id)){
					$mwb_crp_mail = 'crp_signup_email';
					$desc =  "Coupon on Registration for UserID";
					$this->mwb_reffral_custmization($user_id,$desc,$mwb_crp_mail);
					update_user_meta($user_id,'mwb_cpr_user_referred_by',$refree_id);
					$mwb_crp_mail = 'crp_order_email';
					$desc =  "Coupon on Registration for Referral UserID";
					$this->mwb_reffral_custmization($refree_id,$desc,$mwb_crp_mail);
	...
	public function mwb_reffral_custmization($user_id,$desc,$mwb_crp_mail) {
		...
		if($this->mwb_cpr_create_coupons($mwb_cpr_code,$mwb_cpr_coupon_amount,$user_id,$mwb_cpr_discount_type,$expirydate,$coupon_description)) {
			...
			if(empty($expirydate))
			{
				$expirydate = __("No Expiration","coupon-referral-program");
			}
			// print_r($user_id);die("hello");
			if($mwb_crp_mail == 'crp_signup_email') {

				$customer_email = WC()->mailer()->emails['crp_signup_email'];
				$email_status = $customer_email->trigger( $user_id , $mwb_cpr_code,$coupon_amount_with_css,$expirydate);
			}
			
			if($mwb_crp_mail == 'crp_order_email') {
				$customer_email = WC()->mailer()->emails[$mwb_crp_mail];
				$email_status = $customer_email->trigger( $user_id, $mwb_cpr_code,$coupon_amount_with_css,$expirydate);
			}
		...
		if(!is_admin()){
			if(!is_user_logged_in()){
				...
				if(isset($_GET['ref']) && !empty($_GET['ref'])){
					...
					if(isset($mwb_cpr_ref_link_expiry) && !empty($mwb_cpr_ref_link_expiry) && !empty($referral_key)){
						setcookie( 'mwb_cpr_cookie_set', $referral_key, time() + (86400 * $mwb_cpr_ref_link_expiry), "/" );
					}
				}
			}
		}
	...
	public function mwb_crp_wsc_compatibility($global_cpn){
		if(is_user_logged_in()) {
			global $current_user;
			$current_user_coupon = array();
			...
			foreach($coupons as $coupon){
				$user_coupon = get_post_meta($coupon->ID,'coupon_created_to',true);
				if(!$this->check_is_enable_coupon_storewide($coupon->ID)) {
					
					if($user_coupon == $user_id){
						$global_cpn[] = $coupon;
					}
				}
			}
	...
	public function check_is_enable_coupon_storewide($coupon_id) {
		$mwb_is_enable = false;
		$is_enable_coupon = get_post_meta($coupon_id,'sc_is_visible_storewide',true);
		if(!empty($is_enable_coupon) && $is_enable_coupon == "yes") {
			$mwb_is_enable = true;
		}
